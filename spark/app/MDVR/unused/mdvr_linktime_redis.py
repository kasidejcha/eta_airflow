...
def link_timestamp(link,time_path):
    ...
    c = pd.DataFrame(check.groupby('PlateNo')['link_timestamp'].max())
    threshold = (pd.to_datetime(c.max())-timedelta(minutes=30)).dt.strftime("%Y-%m-%d %H:%M:%S").iloc[0]
    c = c[c.link_timestamp > threshold].sort_values('link_timestamp', ascending=True).reset_index()
    c = c.iloc[0]
    with open(time_path, 'w') as f:
        f.write(c.link_timestamp)


r = redis.Redis(host='10.15.5.25', port=6379, db=0)
r.auth('eYVX7EwVmmxKPCDmwMtyKVge8oLd2t81')
...
if len(link) != 0:
    ...
    if data != None:
        # filtered
        old_link = pd.read_json(data)
        link = link[~link.id.isin(old_link.id.tolist())]
        link['upload_time_utc'] = [datetime.now().strftime("%Y-%m-%d %H:%M:%S")]*len(link)
