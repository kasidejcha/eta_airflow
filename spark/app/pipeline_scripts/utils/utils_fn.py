...
def find_direction(station_num, route_lat_lon, gps_data_lat_lon, gps_data):
    ...
    for i in range(len(gps_data_lat_lon)):
        ...
        for j in range(len(station_num)):
            coords_2 = route_lat_lon[j] # first 4 stations
        ...
        if dis < 120 and closest_stop_num < 10:
            ...
            if len(station_index)==3 and go_count_diff < 40:
                ...
                if (s in seq): # stations must be in sequence Ex.(12),(23)
                    ...
                    diff_pos = []
                    seq_list.append(s)
                    for k in range(40): # look at points around itself to find closest index to starting station
                        m=40
                        point = i-m+k
                    ...
                    if len(start_index) == 0:
                        ...
                        find_end = 1
                    if len(start_index)>=1 and abs(start_index[-1]-(pos))>m: # 40 points --> remove near by points
                        start_index.append(pos)
        ...
        if dis < 120 and closest_stop_num > station_num[-10] and find_end == 1:
            ...
            if len(end_station_index)==3 and go_count_diff<40:
                # print('seq:', station_index)
